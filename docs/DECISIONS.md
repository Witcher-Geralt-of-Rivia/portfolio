...
Status: Accepted, and its open question is now closed by D-094
Stage: 9C4.0
...
## D-094 - An asset code is issued, never typed

Status: Accepted
Stage: 9C4.A
Resolves: the open half of D-090

### Decision
`assetCode` is generated by the domain. A visitor never enters one and never
edits one.

```
create form   Vehicle class   Model   Odometer
edit form     Vehicle class   Model   Odometer
```

The canonical seed ends at `MTR-024`, so the first vehicle a visitor creates is
`MTR-025`, the next `MTR-026`, and so on.

Generation reads the **highest numeric suffix among the vehicles that actually
exist**, not the size of the collection, and then checks the candidate against
the codes in use before issuing it. A suffix, once issued, is never reissued.

The four derived fields stay derived: `status`, `currentContractId`,
`currentReservationId` and `activeMaintenanceId` have no control anywhere in the
Fleet module and are written only by the recomputation every service already
performs.

### Reason
An asset code is how a fleet refers to a machine on a worksheet, on a key fob
and over a radio. Two vehicles sharing one is a fleet that cannot be talked
about, and a visitor typing the identity is the shortest path to that.

Length is the wrong source. The demo's store is a local database the visitor can
mutate and reset, so after a single deletion the count and the highest code
disagree and the next allocation lands on a code already in use. The highest
suffix in use is the only thing that answers "what has been handed out". The
collision check on top of it covers the case the suffix alone cannot: a
differently padded twin such as `MTR-0025`, which parses to the same number.

Never reusing a suffix matters for the same reason the code exists. A code that
came back after a deletion would make an old worksheet quoting it point at a
different machine.

Class and model are validated together because `MODELS_BY_CLASS` already says a
model belongs to exactly one class. The form narrows its options to that class
and re-homes the model when the class changes, and the service checks the pair
again, because a form is one caller of a domain rule.

### Consequence
`services/vehicles.ts` is the eleventh service and the first that allocates a
domain identity. It exports `createVehicle`, `updateVehicle` and the pure
`nextAssetCode`, which the QA exercises directly on worlds the seed cannot
produce: an empty fleet, a gap in the middle, a deleted top record and a padded
twin.

There is no vehicle workflow wrapper and none is missing. No entry in the frozen
`AutomationTrigger` union names a vehicle, so there is no application boundary
here for one to stand at (D-088).

Odometer validation is stated three times over: the input constrains it, the
