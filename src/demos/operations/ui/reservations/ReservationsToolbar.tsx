...
/**
 * Operations demo: the Reservations toolbar.
 *
 * The Leads and Customers toolbar with two filters and one sort. Inline on a
 * wide screen, behind a Filters button in a sheet on a phone, and the approved
 * select for every control.
 *
 * The status filter carries live counts, because "how many drafts are waiting"
...
type Props = {
  ...
  mayWrite: boolean;
  filtersOpen: boolean;
  onPatch: (next: Partial<ReservationListQuery>) => void;
  onClear: () => void;
  onOpenFilters: () => void;
  onCloseFilters: () => void;
  onCreate: () => void;
    ...
    <div className="ops-leads__toolbar">
      ...
      <div className="ops-leads__result">
        <p className="ops-leads__count" aria-live="polite">
          {total === null ? " " : `${total} ${total === 1 ? "reservation" : "reservations"}`}
        </p>
        {!isDefault && (
      ...
      {filtersOpen && (
        <OpsOverlay variant="sheet" label="Filter and sort reservations" onClose={onCloseFilters}>
    ...
    <>
      {wrap(
        <OpsSelect
          ...
          onChange={(v) => onPatch({ status: v as ReservationListQuery["status"] })}
      ...
      {wrap(
        <OpsSelect
          ...
          onChange={(v) => onPatch({ vehicleClass: v as ReservationListQuery["vehicleClass"] })}
